...
// PostAllocationsRequest is operation request value.
type PostAllocationsRequest struct {
	// Name is a required `Name` parameter in query.
	// must be less than 100 characters and use only numbers, letters, underscores, hyphens, and periods
	Name    string
	Version *string  // Version is an optional `version` parameter in query.
}

// encode creates *http.Request for request data.
func (request *PostAllocationsRequest) encode(ctx context.Context, baseURL string) (*http.Request, error) {
...
// PostAllocationsResponse is operation response value.
type PostAllocationsResponse struct {
	StatusCode               int
	RawBody                  []byte                                           // RawBody contains read bytes of response body.
	ValueOK                  *PostAllocationsResponseValueOK                  // ValueOK is a value of 200 OK response.
...
func (result *PostAllocationsResponse) decode(resp *http.Response) error {
	var err error
	...
	case http.StatusOK:
		...
		if err != nil {
			err = fmt.Errorf("failed to decode 'post /allocations' OK response: %w", err)
		}
	case http.StatusBadRequest:
		err = json.NewDecoder(body).Decode(&result.ValueBadRequest)
		if err != nil {
			err = fmt.Errorf("failed to decode 'post /allocations' BadRequest response: %w", err)
		}
	case http.StatusUnauthorized:
		err = json.NewDecoder(body).Decode(&result.ValueUnauthorized)
		if err != nil {
			err = fmt.Errorf("failed to decode 'post /allocations' Unauthorized response: %w", err)
		}
	case http.StatusForbidden:
		err = json.NewDecoder(body).Decode(&result.ValueForbidden)
		if err != nil {
			err = fmt.Errorf("failed to decode 'post /allocations' Forbidden response: %w", err)
		}
	case http.StatusNotFound:
		err = json.NewDecoder(body).Decode(&result.ValueNotFound)
		if err != nil {
			err = fmt.Errorf("failed to decode 'post /allocations' NotFound response: %w", err)
		}
	case http.StatusInternalServerError:
		err = json.NewDecoder(body).Decode(&result.ValueInternalServerError)
		if err != nil {
			err = fmt.Errorf("failed to decode 'post /allocations' InternalServerError response: %w", err)
		}
	default:
...
// PostAllocations performs REST operation.
func (c *Client) PostAllocations(ctx context.Context, request PostAllocationsRequest) (result PostAllocationsResponse, err error) {
	if c.InstrumentCtxFunc != nil {
		ctx = c.InstrumentCtxFunc(ctx, http.MethodPost, "/allocations", &request)
	}
