...
// GetSubscriptionsRequest is operation request value.
...
func (request *GetSubscriptionsRequest) encode(ctx context.Context, baseURL string) (*http.Request, error) {
	requestURI := baseURL + "/subscriptions"
...
// GetSubscriptionsResponse is operation response value.
type GetSubscriptionsResponse struct {
	StatusCode               int
	RawBody                  []byte                                           // RawBody contains read bytes of response body.
	ValueOK                  *GetSubscriptionsResponseValueOK                 // ValueOK is a value of 200 OK response.
...
func (result *GetSubscriptionsResponse) decode(resp *http.Response) error {
	var err error
	...
	case http.StatusOK:
		...
		if err != nil {
			err = fmt.Errorf("failed to decode 'get /subscriptions' OK response: %w", err)
		}
	case http.StatusBadRequest:
		err = json.NewDecoder(body).Decode(&result.ValueBadRequest)
		if err != nil {
			err = fmt.Errorf("failed to decode 'get /subscriptions' BadRequest response: %w", err)
		}
	case http.StatusUnauthorized:
		err = json.NewDecoder(body).Decode(&result.ValueUnauthorized)
		if err != nil {
			err = fmt.Errorf("failed to decode 'get /subscriptions' Unauthorized response: %w", err)
		}
	case http.StatusForbidden:
		err = json.NewDecoder(body).Decode(&result.ValueForbidden)
		if err != nil {
			err = fmt.Errorf("failed to decode 'get /subscriptions' Forbidden response: %w", err)
		}
	case http.StatusNotFound:
		err = json.NewDecoder(body).Decode(&result.ValueNotFound)
		if err != nil {
			err = fmt.Errorf("failed to decode 'get /subscriptions' NotFound response: %w", err)
		}
	case http.StatusInternalServerError:
		err = json.NewDecoder(body).Decode(&result.ValueInternalServerError)
		if err != nil {
			err = fmt.Errorf("failed to decode 'get /subscriptions' InternalServerError response: %w", err)
		}
	default:
...
// GetSubscriptions performs REST operation.
func (c *Client) GetSubscriptions(ctx context.Context, request GetSubscriptionsRequest) (result GetSubscriptionsResponse, err error) {
	if c.InstrumentCtxFunc != nil {
		ctx = c.InstrumentCtxFunc(ctx, http.MethodGet, "/subscriptions", &request)
	}
