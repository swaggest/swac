...
// GetErrataAdvisoryIDSystemsRequest is operation request value.
type GetErrataAdvisoryIDSystemsRequest struct {
...
func (request *GetErrataAdvisoryIDSystemsRequest) encode(ctx context.Context, baseURL string) (*http.Request, error) {
	requestURI := baseURL + "/errata/" + url.PathEscape(request.AdvisoryID) + "/systems"
...
// GetErrataAdvisoryIDSystemsResponse is operation response value.
type GetErrataAdvisoryIDSystemsResponse struct {
	StatusCode               int
	RawBody                  []byte                                           // RawBody contains read bytes of response body.
	ValueOK                  *SystemListMock                                  // ValueOK is a value of 200 OK response.
...
func (result *GetErrataAdvisoryIDSystemsResponse) decode(resp *http.Response) error {
	var err error
	...
	case http.StatusOK:
		...
		if err != nil {
			err = fmt.Errorf("failed to decode 'get /errata/{AdvisoryID}/systems' OK response: %w", err)
		}
	case http.StatusBadRequest:
		err = json.NewDecoder(body).Decode(&result.ValueBadRequest)
		if err != nil {
			err = fmt.Errorf("failed to decode 'get /errata/{AdvisoryID}/systems' BadRequest response: %w", err)
		}
	case http.StatusUnauthorized:
		err = json.NewDecoder(body).Decode(&result.ValueUnauthorized)
		if err != nil {
			err = fmt.Errorf("failed to decode 'get /errata/{AdvisoryID}/systems' Unauthorized response: %w", err)
		}
	case http.StatusForbidden:
		err = json.NewDecoder(body).Decode(&result.ValueForbidden)
		if err != nil {
			err = fmt.Errorf("failed to decode 'get /errata/{AdvisoryID}/systems' Forbidden response: %w", err)
		}
	case http.StatusNotFound:
		err = json.NewDecoder(body).Decode(&result.ValueNotFound)
		if err != nil {
			err = fmt.Errorf("failed to decode 'get /errata/{AdvisoryID}/systems' NotFound response: %w", err)
		}
	case http.StatusInternalServerError:
		err = json.NewDecoder(body).Decode(&result.ValueInternalServerError)
		if err != nil {
			err = fmt.Errorf("failed to decode 'get /errata/{AdvisoryID}/systems' InternalServerError response: %w", err)
		}
	default:
...
// GetErrataAdvisoryIDSystems performs REST operation.
func (c *Client) GetErrataAdvisoryIDSystems(ctx context.Context, request GetErrataAdvisoryIDSystemsRequest) (result GetErrataAdvisoryIDSystemsResponse, err error) {
	if c.InstrumentCtxFunc != nil {
		ctx = c.InstrumentCtxFunc(ctx, http.MethodGet, "/errata/{AdvisoryID}/systems", &request)
	}
