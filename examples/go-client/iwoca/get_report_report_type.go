...
func (request *GetReportReportTypeRequest) encode(ctx context.Context, baseURL string) (*http.Request, error) {
	requestURI := baseURL + "/report/" + url.PathEscape(request.ReportType) + "/"
...
func (result *GetReportReportTypeResponse) decode(resp *http.Response) error {
	var err error
	...
	case http.StatusOK:
		...
		if err != nil {
			err = fmt.Errorf("failed to decode 'get /report/{report_type}/' OK response: %w", err)
		}
	default:
...
// GetReportReportType performs REST operation.
func (c *Client) GetReportReportType(ctx context.Context, request GetReportReportTypeRequest) (result GetReportReportTypeResponse, err error) {
	if c.InstrumentCtxFunc != nil {
		ctx = c.InstrumentCtxFunc(ctx, http.MethodGet, "/report/{report_type}/", &request)
	}
