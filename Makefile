...
docker-phar:
	...
	@composer install --no-dev --prefer-dist;rm -rf tests/;rm -rf examples/;rm ./swac;rm ./swac.tar.gz;docker run -v $(shell pwd):/code -v $$HOME/.cache/composer/phar-composer.phar:/phar-composer.phar -w /code --rm composer:1.10 php -d phar.readonly=off /phar-composer.phar build;mv ./swac.phar ./swac;chmod +x ./swac;tar -zcvf ./swac.tar.gz ./swac;git reset --hard;composer install
