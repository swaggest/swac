...
// GetImagesCsetContentSetRequest is operation request value.
type GetImagesCsetContentSetRequest struct {
...
func (request *GetImagesCsetContentSetRequest) encode(ctx context.Context, baseURL string) (*http.Request, error) {
	requestURI := baseURL + "/images/cset/" + url.PathEscape(request.ContentSet)
...
// GetImagesCsetContentSetResponse is operation response value.
type GetImagesCsetContentSetResponse struct {
	StatusCode               int
	RawBody                  []byte                                           // RawBody contains read bytes of response body.
	ValueOK                  *GetImagesCsetContentSetResponseValueOK          // ValueOK is a value of 200 OK response.
...
func (result *GetImagesCsetContentSetResponse) decode(resp *http.Response) error {
	var err error
	...
	case http.StatusOK:
		...
		if err != nil {
			err = fmt.Errorf("failed to decode 'get /images/cset/{ContentSet}' OK response: %w", err)
		}
	case http.StatusBadRequest:
		err = json.NewDecoder(body).Decode(&result.ValueBadRequest)
		if err != nil {
			err = fmt.Errorf("failed to decode 'get /images/cset/{ContentSet}' BadRequest response: %w", err)
		}
	case http.StatusUnauthorized:
		err = json.NewDecoder(body).Decode(&result.ValueUnauthorized)
		if err != nil {
			err = fmt.Errorf("failed to decode 'get /images/cset/{ContentSet}' Unauthorized response: %w", err)
		}
	case http.StatusForbidden:
		err = json.NewDecoder(body).Decode(&result.ValueForbidden)
		if err != nil {
			err = fmt.Errorf("failed to decode 'get /images/cset/{ContentSet}' Forbidden response: %w", err)
		}
	case http.StatusNotFound:
		err = json.NewDecoder(body).Decode(&result.ValueNotFound)
		if err != nil {
			err = fmt.Errorf("failed to decode 'get /images/cset/{ContentSet}' NotFound response: %w", err)
		}
	case http.StatusInternalServerError:
		err = json.NewDecoder(body).Decode(&result.ValueInternalServerError)
		if err != nil {
			err = fmt.Errorf("failed to decode 'get /images/cset/{ContentSet}' InternalServerError response: %w", err)
		}
	default:
...
// GetImagesCsetContentSet performs REST operation.
func (c *Client) GetImagesCsetContentSet(ctx context.Context, request GetImagesCsetContentSetRequest) (result GetImagesCsetContentSetResponse, err error) {
	if c.InstrumentCtxFunc != nil {
		ctx = c.InstrumentCtxFunc(ctx, http.MethodGet, "/images/cset/{ContentSet}", &request)
	}
