...
// GetAllocationsUuidRequest is operation request value.
type GetAllocationsUuidRequest struct {
...
func (request *GetAllocationsUuidRequest) encode(ctx context.Context, baseURL string) (*http.Request, error) {
	requestURI := baseURL + "/allocations/" + url.PathEscape(request.Uuid)

	query := make(url.Values, 1)

	if request.Include != nil {
		query.Set("include", *request.Include)
	}
...
// GetAllocationsUuidResponse is operation response value.
type GetAllocationsUuidResponse struct {
	StatusCode               int
	RawBody                  []byte                                           // RawBody contains read bytes of response body.
	ValueOK                  *GetAllocationsUuidResponseValueOK               // ValueOK is a value of 200 OK response.
...
func (result *GetAllocationsUuidResponse) decode(resp *http.Response) error {
	var err error
	...
	case http.StatusOK:
		...
		if err != nil {
			err = fmt.Errorf("failed to decode 'get /allocations/{uuid}' OK response: %w", err)
		}
	case http.StatusBadRequest:
		err = json.NewDecoder(body).Decode(&result.ValueBadRequest)
		if err != nil {
			err = fmt.Errorf("failed to decode 'get /allocations/{uuid}' BadRequest response: %w", err)
		}
	case http.StatusUnauthorized:
		err = json.NewDecoder(body).Decode(&result.ValueUnauthorized)
		if err != nil {
			err = fmt.Errorf("failed to decode 'get /allocations/{uuid}' Unauthorized response: %w", err)
		}
	case http.StatusForbidden:
		err = json.NewDecoder(body).Decode(&result.ValueForbidden)
		if err != nil {
			err = fmt.Errorf("failed to decode 'get /allocations/{uuid}' Forbidden response: %w", err)
		}
	case http.StatusNotFound:
		err = json.NewDecoder(body).Decode(&result.ValueNotFound)
		if err != nil {
			err = fmt.Errorf("failed to decode 'get /allocations/{uuid}' NotFound response: %w", err)
		}
	case http.StatusInternalServerError:
		err = json.NewDecoder(body).Decode(&result.ValueInternalServerError)
		if err != nil {
			err = fmt.Errorf("failed to decode 'get /allocations/{uuid}' InternalServerError response: %w", err)
		}
	default:
...
// GetAllocationsUuid performs REST operation.
func (c *Client) GetAllocationsUuid(ctx context.Context, request GetAllocationsUuidRequest) (result GetAllocationsUuidResponse, err error) {
	if c.InstrumentCtxFunc != nil {
		ctx = c.InstrumentCtxFunc(ctx, http.MethodGet, "/allocations/{uuid}", &request)
	}
