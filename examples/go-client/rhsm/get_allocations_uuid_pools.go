...
// GetAllocationsUuidPoolsRequest is operation request value.
type GetAllocationsUuidPoolsRequest struct {
	...
	Offset *int64
	// Future is an optional `future` parameter in query.
...
func (request *GetAllocationsUuidPoolsRequest) encode(ctx context.Context, baseURL string) (*http.Request, error) {
	requestURI := baseURL + "/allocations/" + url.PathEscape(request.Uuid) + "/pools"
	...
	if request.Future != nil {
		query.Set("future", strconv.FormatBool(*request.Future))
	}
...
// GetAllocationsUuidPoolsResponse is operation response value.
type GetAllocationsUuidPoolsResponse struct {
	StatusCode               int
...
func (result *GetAllocationsUuidPoolsResponse) decode(resp *http.Response) error {
	var err error
	...
	case http.StatusOK:
		...
		if err != nil {
			err = fmt.Errorf("failed to decode 'get /allocations/{uuid}/pools' OK response: %w", err)
		}
	case http.StatusBadRequest:
		err = json.NewDecoder(body).Decode(&result.ValueBadRequest)
		if err != nil {
			err = fmt.Errorf("failed to decode 'get /allocations/{uuid}/pools' BadRequest response: %w", err)
		}
	case http.StatusUnauthorized:
		err = json.NewDecoder(body).Decode(&result.ValueUnauthorized)
		if err != nil {
			err = fmt.Errorf("failed to decode 'get /allocations/{uuid}/pools' Unauthorized response: %w", err)
		}
	case http.StatusForbidden:
		err = json.NewDecoder(body).Decode(&result.ValueForbidden)
		if err != nil {
			err = fmt.Errorf("failed to decode 'get /allocations/{uuid}/pools' Forbidden response: %w", err)
		}
	case http.StatusNotFound:
		err = json.NewDecoder(body).Decode(&result.ValueNotFound)
		if err != nil {
			err = fmt.Errorf("failed to decode 'get /allocations/{uuid}/pools' NotFound response: %w", err)
		}
	case http.StatusInternalServerError:
		err = json.NewDecoder(body).Decode(&result.ValueInternalServerError)
		if err != nil {
			err = fmt.Errorf("failed to decode 'get /allocations/{uuid}/pools' InternalServerError response: %w", err)
		}
	default:
...
// GetAllocationsUuidPools performs REST operation.
func (c *Client) GetAllocationsUuidPools(ctx context.Context, request GetAllocationsUuidPoolsRequest) (result GetAllocationsUuidPoolsResponse, err error) {
	if c.InstrumentCtxFunc != nil {
		ctx = c.InstrumentCtxFunc(ctx, http.MethodGet, "/allocations/{uuid}/pools", &request)
	}
