...
// GetErrataAdvisoryIDPackagesRequest is operation request value.
type GetErrataAdvisoryIDPackagesRequest struct {
	// AdvisoryID is a required `AdvisoryID` parameter in path.
...
func (request *GetErrataAdvisoryIDPackagesRequest) encode(ctx context.Context, baseURL string) (*http.Request, error) {
	requestURI := baseURL + "/errata/" + url.PathEscape(request.AdvisoryID) + "/packages"
...
// GetErrataAdvisoryIDPackagesResponse is operation response value.
type GetErrataAdvisoryIDPackagesResponse struct {
	StatusCode               int
	RawBody                  []byte                                           // RawBody contains read bytes of response body.
	ValueOK                  *PkgListMock                                     // ValueOK is a value of 200 OK response.
...
func (result *GetErrataAdvisoryIDPackagesResponse) decode(resp *http.Response) error {
	var err error
	...
	case http.StatusOK:
		...
		if err != nil {
			err = fmt.Errorf("failed to decode 'get /errata/{AdvisoryID}/packages' OK response: %w", err)
		}
	case http.StatusBadRequest:
		err = json.NewDecoder(body).Decode(&result.ValueBadRequest)
		if err != nil {
			err = fmt.Errorf("failed to decode 'get /errata/{AdvisoryID}/packages' BadRequest response: %w", err)
		}
	case http.StatusUnauthorized:
		err = json.NewDecoder(body).Decode(&result.ValueUnauthorized)
		if err != nil {
			err = fmt.Errorf("failed to decode 'get /errata/{AdvisoryID}/packages' Unauthorized response: %w", err)
		}
	case http.StatusForbidden:
		err = json.NewDecoder(body).Decode(&result.ValueForbidden)
		if err != nil {
			err = fmt.Errorf("failed to decode 'get /errata/{AdvisoryID}/packages' Forbidden response: %w", err)
		}
	case http.StatusNotFound:
		err = json.NewDecoder(body).Decode(&result.ValueNotFound)
		if err != nil {
			err = fmt.Errorf("failed to decode 'get /errata/{AdvisoryID}/packages' NotFound response: %w", err)
		}
	case http.StatusInternalServerError:
		err = json.NewDecoder(body).Decode(&result.ValueInternalServerError)
		if err != nil {
			err = fmt.Errorf("failed to decode 'get /errata/{AdvisoryID}/packages' InternalServerError response: %w", err)
		}
	default:
...
// GetErrataAdvisoryIDPackages performs REST operation.
func (c *Client) GetErrataAdvisoryIDPackages(ctx context.Context, request GetErrataAdvisoryIDPackagesRequest) (result GetErrataAdvisoryIDPackagesResponse, err error) {
	if c.InstrumentCtxFunc != nil {
		ctx = c.InstrumentCtxFunc(ctx, http.MethodGet, "/errata/{AdvisoryID}/packages", &request)
	}
